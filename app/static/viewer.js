...
const dimensionNames = { willingness: "Willingness", adaptability: "Adaptability", respect: "Respect", intelligence: "Intelligence", application: "Application", physical_ability: "Physical Ability" };
const state = { session: null, accounts: [], journeys: [], journeyId: "", data: null, tab: "results", attendanceTab: "recruits", resultsActivity: "overall", profileId: "" };
...
async function loadJourney() {
  state.data = await api(`/api/view/journeys/${state.journeyId}`);
  if (!state.data.recruits.some(item => item.id === state.profileId)) state.profileId = state.data.recruits[0]?.id || "";
  await render();
}

$("#viewerJourney").onchange = async event => { state.journeyId = event.target.value; state.profileId = ""; await loadJourney(); };
...
function renderAttendance() {
  ...
  const evaluatorGroup = item => item.present ? (item.role === "overall" ? 0 : 1) : (item.role === "overall" ? 2 : 3);
  const evaluators = [...state.data.evaluators].sort((a, b) => evaluatorGroup(a) - evaluatorGroup(b) || a.name.localeCompare(b.name));
  const recruitTable = `<div class="table-wrap"><table><thead><tr><th>Photo</th><th>Name</th><th>Phone number</th><th>Date of birth</th><th>Present</th><th>Arrival time</th><th>Attendance comment</th></tr></thead><tbody>${recruits.map(item => { const photo = `/api/view/journeys/${state.journeyId}/recruits/${item.id}/photo`; return `<tr class="${item.present ? "" : "inactive"}"><td>${item.hasPhoto ? `<button class="photo-zoom-trigger" data-view-photo="${photo}" data-photo-name="${h(item.name)}"><img class="avatar" src="${photo}" alt="${h(item.name)}"></button>` : `<span class="avatar placeholder">${h(item.name[0])}</span>`}</td><td><strong>${h(item.name)}</strong></td><td>${h(item.phoneNumber || "—")}</td><td>${h(item.dateOfBirth || "—")}</td><td><span class="status-pill ${item.present ? "completed" : "warning"}">${item.present ? "Present" : "Absent"}</span></td><td>${item.arrivalTime ? h(localDateTime(item.arrivalTime)) : "—"}</td><td>${h(item.attendanceComment || "—")}</td></tr>`; }).join("")}</tbody></table></div>`;
  const evaluatorTable = `<div class="table-wrap"><table><thead><tr><th>Name</th><th>Present</th><th>Role</th><th>Mandatory room</th></tr></thead><tbody>${evaluators.map(item => `<tr class="${item.present ? "" : "inactive"}"><td><strong>${h(item.name)}</strong></td><td><span class="status-pill ${item.present ? "completed" : "warning"}">${item.present ? "Present" : "Absent"}</span></td><td><span class="role-badge ${item.role}">${h(item.role)}</span></td><td>${item.mandatoryRoom ? `Room ${item.mandatoryRoom}` : "—"}</td></tr>`).join("")}</tbody></table></div>`;
  host.innerHTML = `${sectionHeading("Confirmed roster", "Attendance", "Current confirmed attendance in view-only mode.")}<div class="tabs"><button data-attendance="recruits" class="${state.attendanceTab === "recruits" ? "active" : ""}">Recruits</button><button data-attendance="evaluators" class="${state.attendanceTab === "evaluators" ? "active" : ""}">Evaluators</button></div><div class="panel">${state.attendanceTab === "recruits" ? recruitTable : evaluatorTable}</div>`;
...
function renderResults() {
  ...
  host.innerHTML = `${sectionHeading("Scores", "Results & rankings", "Rankings, dimensions, and activity statistics.", `<a class="button secondary" href="/api/view/journeys/${state.journeyId}/results.xlsx">Results & profiles report</a><a class="button primary" href="/api/view/journeys/${state.journeyId}/export.xlsx">Full Journee report</a>`)}<div class="panel"><div class="tabs">${tabs}</div>${table}</div>`;
  $$('[data-result]', host).forEach(button => button.onclick = () => { state.resultsActivity = button.dataset.result; renderResults(); });
  $$(".result-profile", host).forEach(button => button.onclick = async () => { state.profileId = button.dataset.id; state.tab = "profiles"; await render(); });
}
...
function profileHtml(profile) {
  ...
  const dimensionItems = dimensionOrder.map(code => ({ name: dimensionNames[code], score: result.dimensions?.[code]?.score || 0 }));
...
async function showEvaluation(profile, code, index) {
  ...
  if (detail?.id) detail = await api(`/api/view/journeys/${state.journeyId}/submissions/${detail.id}`);
