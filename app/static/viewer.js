...
const officialScoreMaximum = Number(systemConfiguration?.scoreMaximum || 20);
const activityScoreMaximum = Number(systemConfiguration?.activityScoreMaximum || 5);
const hasGeneralAssessment = systemConfiguration?.features?.generalAssessment !== false && Boolean(systemConfiguration?.generalFactors?.length);
const participantTerm = systemConfiguration?.terminology?.participant || "Participant";
const generalSectionTitle = hasGeneralAssessment ? "General assessment" : `${participantTerm} notes`;
const categoryName = role => systemConfiguration?.assessorCategories?.find(item => item.key === role)?.name || role;
...
function renderAttendance() {
  ...
  const evaluatorTable = `<div class="table-wrap"><table><thead><tr><th>Name</th><th>Journee</th><th>Present</th><th>Role</th><th>Mandatory room</th></tr></thead><tbody>${evaluators.map(item => `<tr class="${item.present ? "" : "inactive"}"><td><strong>${h(item.name)}</strong></td><td>${h(item.journeyName)}</td><td><span class="status-pill ${item.present ? "completed" : "warning"}">${item.present ? "Present" : "Absent"}</span></td><td><span class="role-badge ${item.role}">${h(categoryName(item.role))}</span></td><td>${item.mandatoryRoom ? `Room ${item.mandatoryRoom}` : "—"}</td></tr>`).join("")}</tbody></table></div>`;
...
function activityTable(rows, code, average) {
  return `<p class="muted">Activity average: <strong>${fmt(average)} /${activityScoreMaximum}</strong></p><div class="table-wrap"><table><thead><tr><th>Rank</th><th>Recruit</th><th>Journee</th><th>Grade /${activityScoreMaximum}</th><th>Submissions</th><th>Status</th></tr></thead><tbody>${rows.map(row => { const item = row.activities[code]; return `<tr><td><span class="rank-number">${item.rank ?? "—"}</span></td><td><button class="button ghost small result-profile" data-key="${row.profileKey}">${h(row.name)}</button></td><td>${h(row.journeyName)}</td><td><strong>${fmt(item.score)}</strong></td><td>${item.submitted}/${item.expected}</td><td><span class="status-pill ${item.complete ? "completed" : "warning"}">${item.complete ? "Complete" : "Incomplete"}</span></td></tr>`; }).join("")}</tbody></table></div>`;
}
function renderResults() {
  const results = state.data.results;
  const tabs = [`<button data-result="overall" class="${state.resultsActivity === "overall" ? "active" : ""}">Overall /${officialScoreMaximum}</button>`, ...dimensionOrder.map(code => `<button data-result="dimension:${code}" class="${state.resultsActivity === `dimension:${code}` ? "active" : ""}">${h(results.dimensionNames?.[code] || dimensionNames[code])}</button>`), ...state.data.activities.map(item => `<button data-result="activity:${item.code}" class="${state.resultsActivity === `activity:${item.code}` ? "active" : ""}">${h(item.name)}</button>`)].join("");
...
function profileHtml(profile) {
  ...
  const dimensionItems = dimensionOrder.map(code => ({ name: dimensionNames[code], score: dimensionGrade(result.dimensions?.[code]?.score,code), maximum: dimensionMaximums[code] || 5 }));
...
async function renderProfile() {
  ...
  if (assessmentForm) {
    wireBoundedNumberInputs(assessmentForm);
...
function generalAssessmentInputs(assessment) {
  const factors = systemConfiguration?.generalFactors || [];
  if (!factors.length || systemConfiguration?.features?.generalAssessment === false) {
...
function showDimension(profile, code) {
  ...
  const sections = breakdown.activities.map(activity => `<section class="dimension-activity-section"><div class="panel-header"><div><h3>${h(activity.name)}</h3><p class="muted">Activity grade: ${fmt(activity.activityScore)} /${activityScoreMaximum}</p></div></div><div class="dimension-criteria-list">${activity.criteria.map(criterion => `<article class="dimension-criterion"><div class="dimension-criterion-heading"><div><strong>${h(criterion.name)}</strong><p class="muted">${h(criterion.explanation)}</p></div><div class="criterion-math"><span>Weight ${fmt(criterion.weight * 100)}%</span><strong>${criterion.criterionAverage == null ? "—" : `${fmt(criterion.criterionAverage)} /${criterion.maximum}`}</strong><small>Contribution ${fmt(dimensionGrade(criterion.weightedContribution,code))} /${dimensionMaximums[code]||5}</small></div></div><div class="criterion-evaluators">${criterion.evaluators.map(evaluator => `<div class="criterion-evaluator ${evaluator.grade == null ? "missing" : ""}"><div><strong>${h(evaluator.evaluatorName)}</strong><span class="role-badge ${h(evaluator.evaluatorRole)}">${h(categoryName(evaluator.evaluatorRole))}</span></div><div class="criterion-grade"><strong>${evaluator.grade == null ? "Missing" : `${fmt(evaluator.grade)} /${criterion.maximum}`}</strong>${evaluator.rawValue != null ? `<small>Result: ${h(evaluator.rawValue)}${criterion.unit ? ` ${h(criterion.unit)}` : ""}</small>` : ""}<small>${h(statusLabel(evaluator.status))}</small></div></div>`).join("") || `<p class="subtle">No evaluator assignment.</p>`}</div></article>`).join("")}</div></section>`).join("");
