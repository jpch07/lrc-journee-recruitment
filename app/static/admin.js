...
async function renderSettings() {
  ...
  host.innerHTML = `${sectionHeading("Configuration", "Settings, audit & export", "Manage event metadata, evaluator access, archives, and complete data extracts.")}
    ...
    <div class="panel"><div class="panel-header"><div><h2>View-only reports</h2><p class="muted">Download polished snapshots of the admin workspace. The results report includes every ranking and a complete profile sheet for each recruit.</p></div></div><div class="inline-actions"><a class="button primary" href="/api/admin/journeys/${state.journey.id}/export.xlsx">Full Journee report</a><a class="button secondary" href="/api/admin/journeys/${state.journey.id}/results.xlsx">Results & profiles report</a><a class="button ghost" href="/api/admin/journeys/${state.journey.id}/results.csv">Quick results CSV</a><a class="button ghost" href="/api/admin/journeys/${state.journey.id}/photos.zip">Photo ZIP</a><button class="button secondary" id="duplicateCurrent">Duplicate Journee</button></div></div>
