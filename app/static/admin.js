...
function assessorCategories() {
  return state.system?.assessorCategories || [{ key: "overall", name: "Overall", primaryPriority: 0, secondaryPriority: 1 }, { key: "dossard", name: "Dossard", primaryPriority: 1, secondaryPriority: 0 }];
}
...
function accessProfileOptions(selected = "assessor") {
  return (state.system?.accessProfiles || []).filter(item => item.key !== "owner")
    .map(item => `<option value="${h(item.key)}" ${item.key === selected ? "selected" : ""}>${h(item.name)}</option>`).join("");
}
...
function dimensionRadar(result) {
  const items = dimensionOrder.map((code) => ({ code, name: dimensionNames[code], score: dimensionGrade(result.dimensions?.[code]?.score) }));
  ...
  const dataPoints = items.map((item, index) => point(index, Math.max(0, Math.min(5, item.score)) / 5));
  return `<svg class="activity-radar" viewBox="0 0 360 320" role="img" aria-label="Radar chart of the six dimension grades out of five">
    ${[.2,.4,.6,.8,1].map((level) => `<polygon class="radar-grid" points="${polygon(level)}"></polygon>`).join("")}
    ${items.map((_, index) => { const p = point(index, 1); return `<line class="radar-axis" x1="${cx}" y1="${cy}" x2="${p[0]}" y2="${p[1]}"></line>`; }).join("")}
    <polygon class="radar-data" points="${dataPoints.map((p) => p.join(",")).join(" ")}"></polygon>
    ${dataPoints.map((p) => `<circle class="radar-dot" cx="${p[0]}" cy="${p[1]}" r="4"></circle>`).join("")}
    ${items.map((item, index) => { const p = point(index, 1.24); return `<text class="radar-label" x="${p[0]}" y="${p[1]}" text-anchor="middle" dominant-baseline="middle"><tspan x="${p[0]}">${h(item.name)}</tspan><tspan x="${p[0]}" dy="15">${fmt(item.score)}/5</tspan></text>`; }).join("")}
  </svg>`;
}
...
function wireProfile(profile) {
  ...
  const factors = new Map((state.system?.generalFactors || []).map(item => [item.storageKey, item]));
  for (const key of ["punctuality", "respect", "seriousness"]) {
    const input = form.elements[key];
    const factor = factors.get(key);
    input.closest("label").classList.toggle("hidden", !factor || state.system?.features?.generalAssessment === false);
