...
let dimensionOrder = ["willingness", "adaptability", "respect", "intelligence", "application", "physical_ability"];
let dimensionNames = {
...
let dimensionMaximums = Object.fromEntries(dimensionOrder.map(code => [code, 5]));
const dimensionGrade = (value, code) => Number(value || 0) * Number(dimensionMaximums[code] || 5);
const officialScoreMaximum = () => Number(state.system?.scoreMaximum || 20);
const activityScoreMaximum = () => Number(state.system?.activityScoreMaximum || 5);
const hasGeneralAssessment = () => state.system?.features?.generalAssessment !== false && Boolean(state.system?.generalFactors?.length);
const participantTerm = () => state.system?.terminology?.participant || "Participant";
const generalSectionTitle = () => hasGeneralAssessment() ? "General assessment" : `${participantTerm()} notes`;
...
async function renderDashboard() {
  ...
  host.innerHTML = `${sectionHeading("Live operations", data.journey.name, `${data.journey.eventDate} · Updates every five seconds`, `<button class="button primary" id="goProtection">Event-day protection</button><button class="button ghost" id="refreshDashboard">Refresh</button>`)}
    <div class="metric-grid">
      ...
      <div class="metric-card"><small>Evaluator categories</small><div class="split">${assessorCategories().map(category => `<span><strong>${data.evaluatorCategoryCounts?.[category.key] || 0}</strong><small>${h(category.name)}</small></span>`).join("")}</div></div>
      <div class="metric-card"><small>Active activity</small><strong style="font-size:1.15rem">${data.journey.currentActivity ? h(configuredActivityName(data.journey.currentActivity)) : "None"}</strong><span class="subtle">${data.roomPlan ? `${data.roomPlan.rooms.length} rooms published` : "No room plan"}</span></div>
    </div>
    ...
    <div class="panel"><h2>Dimension averages</h2><div class="member-list">${dimensionOrder.map((code) => `<div class="member-chip"><span>${h(data.dimensionNames?.[code] || dimensionNames[code])}</span><strong>${fmt(dimensionGrade(data.dimensionAverages?.[code], code))} /${dimensionMaximums[code]}</strong></div>`).join("")}</div><h3 style="margin-top:20px">Activity averages /${activityScoreMaximum()}</h3><div class="member-list">${Object.entries(data.activityAverages).map(([code, value]) => `<div class="member-chip"><span>${h(configuredActivityName(code))}</span><strong>${fmt(value)}</strong></div>`).join("")}</div><div style="margin-top:18px" class="inline-actions"><button class="button primary" id="quickAttendance">Attendance</button><button class="button secondary" id="quickAssignments">Assignments</button></div></div></div>`;
...
function rankingTable(rows) {
  ...
  return `<div class="table-wrap"><table><thead><tr><th>Rank</th><th>Recruit</th><th>Score /${officialScoreMaximum()}</th><th>Color</th><th>Missing</th></tr></thead><tbody>${rows.map((row) => `<tr><td><span class="rank-number">${row.overallRank}</span></td><td>${h(row.name)}</td><td><strong>${fmt(row.overallScore)}</strong></td><td><span class="color-chip ${row.color}">${h(row.color)}</span></td><td>${row.missingCount}</td></tr>`).join("")}</tbody></table></div>`;
}
...
function assessorCategories() {
  return state.system?.assessorCategories || [{ key: "assessor", name: "Assessor", primaryPriority: 0, secondaryPriority: 0 }];
}

function categoryName(role) {
  return assessorCategories().find(item => item.key === role)?.name || role;
}

function categoryOptions(selected) {
  return assessorCategories().map(item => `<option value="${h(item.key)}" ${item.key === selected ? "selected" : ""}>${h(item.name)}</option>`).join("");
}

function accountPermissionPresets() {
  return (state.system?.accessProfiles || []).filter(item => item.enabled !== false && item.key !== "owner");
}

function defaultAccountPermissionPreset(presets = accountPermissionPresets()) {
  return presets.find(item => item.key === "assessor")
    || presets.find(item => (item.capabilities || []).includes("evaluate"))
    || presets[0]
    || null;
}

function accessProfileOptions(selected = null) {
  const presets = accountPermissionPresets();
  const selectedKey = presets.some(item => item.key === selected)
    ? selected
    : defaultAccountPermissionPreset(presets)?.key;
  return presets.map(item => `<option value="${h(item.key)}" ${item.key === selectedKey ? "selected" : ""}>${h(item.name)}</option>`).join("");
}

function categoryRank(role, priority = "primaryPriority") {
  const category = assessorCategories().find(item => item.key === role);
  return category?.[priority] ?? 999;
}

function configuredActivityName(code) {
  return state.system?.activities?.find(item => item.key === code)?.name || statusLabel(code);
}

function configuredActivity(code) {
  return state.system?.activities?.find(item => item.key === code) || null;
}

function generalAssessmentInputs(assessment) {
  const factors = state.system?.generalFactors || [];
  if (!factors.length || state.system?.features?.generalAssessment === false) {
    return `<p class="subtle">No general assessment factors are configured for this workspace.</p>`;
  }
  const values = assessment?.values || {};
  return `<div class="general-factor-grid">${factors.map((factor) => {
    const value = values[factor.storageKey] ?? assessment?.[factor.storageKey] ?? "";
    return `<label>${h(factor.name)} <small class="field-scale">0–${h(factor.maximum)}</small><input name="factor:${h(factor.storageKey)}" type="number" min="0" max="${h(factor.maximum)}" step="${h(factor.step)}" value="${h(value)}"></label>`;
  }).join("")}</div>`;
}

function sharedAssignmentTargets(code) {
  return (state.system?.activities || []).filter(item => item.assignment?.reuseAssignmentsFrom === code);
}

function evaluatorAttendanceSort(a, b) {
  return Number(b.present) - Number(a.present)
    || categoryRank(a.role) - categoryRank(b.role)
...
function wireEvaluatorAttendanceSearch() {
  ...
  const drawSuggestions = () => {
    ...
    suggestions.innerHTML = matches.map((item) => `<button type="button" data-id="${item.id}" role="option"><span><strong>${h(item.name)}</strong><small>${h(item.fullName || "Full name not recorded")} · ${item.present ? "Present" : "Absent"}</small></span><span class="role-badge ${item.role}">${h(categoryName(item.role))}</span></button>`).join("");
...
function renderAttendanceTable(rows) {
  ...
  const body = rows.map((item) => {
    ...
    return `<tr data-id="${item.id}" class="${item.present ? "" : "inactive"}"><td><strong>${h(item.name)}</strong>${item.mandatoryRoom && !item.present ? `<small class="danger-text"> Required evaluator is absent</small>` : ""}</td><td><input class="attendance-check" type="checkbox" aria-label="${h(item.name)} present" ${item.present ? "checked" : ""}></td><td><span class="role-badge ${item.role}">${h(categoryName(item.role))}</span></td><td>${item.mandatoryRoom ? `Room ${item.mandatoryRoom}` : "—"}</td><td><button class="button danger small delete-person">Delete</button></td></tr>`;
  }).join("");
  target.innerHTML = `<div class="table-wrap"><table class="${isRecruit ? "recruit-attendance-table" : "evaluator-attendance-table"}"><thead><tr>${headers}</tr></thead><tbody>${body}</tbody></table></div>`;
...
async function renderAssignments() {
  ...
  const firstGroupActivity = (state.system?.activities || []).find(item => item.assignment?.mode === "automatic_groups");
  const firstGroupState = firstGroupActivity ? state.journey.activities.find(item => item.code === firstGroupActivity.key) : null;
  const roomsFrozen = firstGroupState && ["open", "closed"].includes(firstGroupState.status);
  const activityTabs = assignmentActivities.map((configured) => {
    const code = configured.key || configured.code;
    const targets = sharedAssignmentTargets(code);
    const name = [configured.name, ...targets.map(item => item.name)].join(" & ");
    return `<button data-activity="${code}" class="${code === state.assignmentActivity ? "active" : ""}">${h(name)}</button>`;
  }).join("");
  const configured = configuredActivity(state.assignmentActivity);
  const reuseTargets = sharedAssignmentTargets(state.assignmentActivity);
  const assignmentName = [configuredActivityName(state.assignmentActivity), ...reuseTargets.map(item => item.name)].join(" & ");
  const roomActivities = (state.system?.activities || []).filter(item => item.assignment?.mode === "automatic_groups");
  const showRooms = state.system?.features?.groupsAndRooms !== false && roomActivities.length > 0;
  const roomPanel = showRooms ? `<div class="panel"><div class="panel-header"><div><h2>Fixed room plan</h2><p class="muted">Used by ${h(roomActivities.map(item => item.name).join(", "))}.</p></div><div class="inline-actions"><label class="compact-field">Rooms<input id="roomCountInput" type="number" min="1" max="100" value="${state.journey.roomCount}"></label><button class="button ghost" id="saveRoomCount">Save room count</button>${state.system?.features?.mandatoryPlacements === false ? "" : `<button class="button secondary" id="mandatoryRooms">Mandatory placements</button>`}<button class="button ghost" id="previewRooms">${previewRooms ? "Regenerate preview" : "Generate preview"}</button>${previewRooms ? `<button class="button primary" id="publishRooms">${roomsFrozen ? "Publish override" : "Publish rooms"}</button>` : ""}</div></div>${state.roomPlan ? renderRoomPlan(state.roomPlan) : `<div class="empty-state"><p>Confirm attendance, then generate the room plan.</p></div>`}</div>` : "";
  host.innerHTML = `${sectionHeading("Distribution", "Rooms & assignments", "Preview changes privately, review warnings, then publish atomically.")}
    ${roomPanel}
    <div class="panel"><div class="tabs">${activityTabs}</div><div class="panel-header"><div><h2>${h(assignmentName)} assignments</h2><p class="muted">${previewRound ? "Showing private preview" : publishedRound ? "Showing evaluator-visible published version" : "No assignment generated"}.${reuseTargets.length ? ` One publication applies the exact pairing to ${reuseTargets.length + 1} activities.` : ""}</p></div><div class="inline-actions"><button class="button ghost" id="previewAssignments">${previewRound ? "Regenerate preview" : configured?.assignment?.mode === "manual" ? "Create manual preview" : "Generate preview"}</button>${previewRound ? `<button class="button primary" id="publishAssignments">${reuseTargets.length ? "Publish shared assignments" : "Publish assignments"}</button>` : ""}</div></div>
  ...
  if ($("#saveRoomCount")) $("#saveRoomCount").onclick = async () => {
  ...
  if ($("#previewRooms")) $("#previewRooms").onclick = async () => actionAndRefresh(`/api/admin/journeys/${state.journey.id}/rooms/preview`, "POST", {}, "Room preview generated.", renderAssignments);
  ...
  if ($("#publishAssignments")) $("#publishAssignments").onclick = async () => actionAndRefresh(`/api/admin/journeys/${state.journey.id}/assignments/${previewRound.id}/publish`, "POST", {}, `${assignmentName} assignments published.`, renderAssignments);
  if ($("#mandatoryRooms")) $("#mandatoryRooms").onclick = mandatoryRoomsDialog;
  if (previewRooms) wireRoomEditors(previewRooms);
  if (previewRound && !configuredActivity(previewRound.activityCode)?.assignment?.reuseAssignmentsFrom) wireAssignmentEditors(previewRound);
}
...
function activityAvailabilityDialog(operation) {
  ...
  const draw = () => {
    ...
    $("#activityAvailabilityList").innerHTML = ordered.map(item => `<label class="mandatory-row activity-availability-row"><div><strong>${h(item.name)}</strong><small>${h(item.fullName || "Full name not recorded")}</small></div><span class="role-badge ${item.role}">${h(categoryName(item.role))}</span><input class="large-check" type="checkbox" data-id="${item.id}" ${values[item.id] ? "checked" : ""}></label>`).join("");
  ...
  input.oninput = () => {
    const matches = evaluatorSearchMatches(evaluators, input.value).slice(0, 8);
    suggestions.innerHTML = matches.map(item => `<button type="button" data-id="${item.id}"><span><strong>${h(item.name)}</strong><small>${values[item.id] ? "Available" : "Unavailable"}</small></span><span class="role-badge ${item.role}">${h(categoryName(item.role))}</span></button>`).join("");
    suggestions.classList.toggle("visible", Boolean(input.value.trim() && matches.length));
...
function wireAssignmentEditors(round) {
  ...
  const drawRow = (container, evaluator, recruit, reason = "") => {
    ...
    container.innerHTML = `<span><strong>${h(evaluator.name)}</strong> <span class="role-badge ${evaluator.role}">${h(categoryName(evaluator.role))}</span></span><span>→</span><span><strong>${h(recruit.name)}</strong>${reason ? `<small class="assignment-note">${h(reason)}</small>` : ""}</span><div class="assignment-row-actions"><button type="button" class="button ghost small edit-assignment">Edit</button><button type="button" class="button ghost small remove-assignment">Remove</button></div>`;
    wireRow(container);
  };
...
function mandatoryRoomsDialog(operation = null) {
  ...
  const drawList = () => {
    ...
    const ordered = [...evaluators].sort((a, b) => {
      ...
      const roleA = categoryRank(a.role);
      const roleB = categoryRank(b.role);
      return roomA - roomB || roleA - roleB || a.name.localeCompare(b.name, undefined, { sensitivity: "base" });
    });
    $("#mandatoryList").innerHTML = ordered.map((item) => `<div class="mandatory-row ${item.present ? "" : "absent"}" data-id="${item.id}"><div><strong>${h(item.name)}</strong><small>${h(item.fullName || "Full name not recorded")} · ${item.present ? "Present" : "Absent"}</small></div><span class="role-badge ${item.role}">${h(categoryName(item.role))}</span><select aria-label="Mandatory room for ${h(item.name)}">${roomOptions(placements[item.id])}</select></div>`).join("");
  ...
  const drawSuggestions = () => {
    const matches = evaluatorSearchMatches(evaluators, input.value).slice(0, 8);
    suggestions.innerHTML = matches.map((item) => `<button type="button" data-id="${item.id}" role="option"><span><strong>${h(item.name)}</strong><small>${h(item.fullName || "Full name not recorded")} · ${item.present ? "Present" : "Absent"}${placements[item.id] ? ` · Room ${placements[item.id]}` : ""}</small></span><span class="role-badge ${item.role}">${h(categoryName(item.role))}</span></button>`).join("");
    suggestions.classList.toggle("visible", Boolean(input.value.trim() && matches.length));
...
async function renderMonitoring() {
  ...
  const parallelGroup = configuredActivity(state.monitoringActivity)?.assignment?.parallelGroup;
  const parallelActivities = parallelGroup ? (state.system?.activities || []).filter(item => item.assignment?.parallelGroup === parallelGroup) : [];
  const parallelStates = parallelActivities.map(item => state.journey.activities.find(stateItem => stateItem.code === item.key)).filter(Boolean);
  const pairStatus = parallelStates.length > 1 && parallelStates.every(item => item.status === parallelStates[0].status) ? parallelStates[0].status : null;
  const pairName = parallelActivities.map(item => item.name).join(" & ");
  const pairButton = parallelActivities.length > 1 && pairStatus
    ? pairStatus === "not_started" ? `<button class="button primary" id="openPair">Open ${h(pairName)}</button>`
  ...
  host.innerHTML = `${sectionHeading("Live submissions", "Activity control & monitoring", "Activities open according to the configured order and parallel groups.", `<button class="button ghost" id="refreshMonitoring">Refresh now</button>`)}
  ...
  if ($("#openPair")) $("#openPair").onclick = () => pairedLifecycleAction(parallelGroup, pairName, "open");
  if ($("#closePair")) $("#closePair").onclick = () => pairedLifecycleAction(parallelGroup, pairName, "close");
  if ($("#reopenPair")) $("#reopenPair").onclick = () => reasonDialog(`Reopen ${pairName}`, "Explain why these activities are being reopened.", (reason) => pairedLifecycleAction(parallelGroup, pairName, "reopen", reason));
...
function monitorTable(data) {
  ...
  return `<div class="table-wrap"><table><thead><tr><th>Evaluator</th><th>Role</th><th>Assigned recruit(s)</th><th>Received</th><th>Evaluations</th></tr></thead><tbody>${data.evaluators.map((item) => `<tr><td>${h(item.name)}</td><td><span class="role-badge ${item.role}">${h(categoryName(item.role))}</span></td><td>${item.tasks.map((task) => h(task.recruitName)).join(", ")}</td><td>${item.tasks.filter((task) => task.submitted).length}/${item.tasks.length}</td><td>${item.tasks.filter((task) => task.submissionId).map((task) => `<button class="button ghost small submission-detail" data-id="${task.submissionId}">Edit ${h(task.recruitName)}</button>`).join(" ") || "—"}</td></tr>`).join("")}</tbody></table></div>`;
}
...
function evaluationCriterionEditor(criterion, value) {
  ...
  if (criterion.inputType === "number") return `<input name="${criterion.key}" type="number" min="0" step="any" inputmode="decimal" value="${h(value ?? "")}" required>`;
  return `<input class="grade-input" name="${criterion.key}" type="number" min="${criterion.minimum ?? 0}" max="${criterion.maximum ?? 5}" step="${criterion.step ?? .1}" inputmode="decimal" value="${h(value ?? "")}" required>`;
}
...
async function showSubmissionDetail(submissionId) {
  try {
    ...
    const values = detail.rubric.kind === "sport" ? detail.raw : detail.responses;
    ...
    correctionForm.onsubmit = async (event) => {
      ...
      for (const criterion of detail.rubric.criteria) {
        if (detail.rubric.kind === "sport") raw[criterion.key] = form.get(criterion.key);
...
async function pairedLifecycleAction(group, name, action, reason = "") {
  if (action === "close" && !confirm(`Close ${name} and lock these evaluations?`)) return;
  await actionAndRefresh(`/api/admin/journeys/${state.journey.id}/activities/parallel/${encodeURIComponent(group)}/${action}`, "POST", { reason }, `${name} ${action === "reopen" ? "reopened" : `${action}ed`} together.`, renderMonitoring);
}
...
async function renderResults() {
  ...
  const tabs = [`<button data-result="overall" class="${state.resultsActivity === "overall" ? "active" : ""}">Overall /${officialScoreMaximum()}</button>`, ...dimensionTabs, ...activityTabs].join("");
...
function activityRadar(result) {
  ...
  const dataPoints = activities.map((activity, index) => point(index, Math.max(0, Math.min(maximum, Number(result.activities?.[activity.code]?.score || 0))) / maximum));
  return `<svg class="activity-radar" viewBox="0 0 320 300" role="img" aria-label="Radar chart of activity grades out of ${maximum}">
    ...
    ${activities.map((activity, index) => { const p = point(index, 1.2); const score = fmt(result.activities?.[activity.code]?.score || 0); return `<text class="radar-label" x="${p[0]}" y="${p[1]}" text-anchor="middle" dominant-baseline="middle"><tspan x="${p[0]}">${h(activity.name)}</tspan><tspan x="${p[0]}" dy="15">${score}/${maximum}</tspan></text>`; }).join("")}
  </svg>`;
}

function dimensionRadar(result) {
  const items = dimensionOrder.map((code) => ({ code, name: dimensionNames[code], score: dimensionGrade(result.dimensions?.[code]?.score, code), maximum: dimensionMaximums[code] || 5 }));
  ...
  const dataPoints = items.map((item, index) => point(index, Math.max(0, Math.min(item.maximum, item.score)) / item.maximum));
  return `<svg class="activity-radar" viewBox="0 0 360 320" role="img" aria-label="Radar chart of dimension grades">
    ${[.2,.4,.6,.8,1].map((level) => `<polygon class="radar-grid" points="${polygon(level)}"></polygon>`).join("")}
    ${items.map((_, index) => { const p = point(index, 1); return `<line class="radar-axis" x1="${cx}" y1="${cy}" x2="${p[0]}" y2="${p[1]}"></line>`; }).join("")}
    <polygon class="radar-data" points="${dataPoints.map((p) => p.join(",")).join(" ")}"></polygon>
    ${dataPoints.map((p) => `<circle class="radar-dot" cx="${p[0]}" cy="${p[1]}" r="4"></circle>`).join("")}
    ${items.map((item, index) => { const p = point(index, 1.24); return `<text class="radar-label" x="${p[0]}" y="${p[1]}" text-anchor="middle" dominant-baseline="middle"><tspan x="${p[0]}">${h(item.name)}</tspan><tspan x="${p[0]}" dy="15">${fmt(item.score)}/${item.maximum}</tspan></text>`; }).join("")}
  </svg>`;
}
...
function showDimensionBreakdown(profile, code) {
  ...
  const activitySections = breakdown.activities.map((activity) => `
    <section class="dimension-activity-section">
      <div class="panel-header"><div><h3>${h(activity.name)}</h3><p class="muted">Activity grade: ${fmt(activity.activityScore)} /${activityScoreMaximum()}</p></div></div>
      <div class="dimension-criteria-list">${activity.criteria.map((criterion) => `
        <article class="dimension-criterion">
          <div class="dimension-criterion-heading"><div><strong>${h(criterion.name)}</strong><p class="muted">${h(criterion.explanation)}</p></div><div class="criterion-math"><span>Weight ${fmt(criterion.weight * 100)}%</span><strong>${criterion.criterionAverage == null ? "—" : `${fmt(criterion.criterionAverage)} /${criterion.maximum}`}</strong><small>Contribution ${fmt(dimensionGrade(criterion.weightedContribution, code))} /${dimensionMaximums[code] || 5}</small></div></div>
          <div class="criterion-evaluators">${criterion.evaluators.length ? criterion.evaluators.map((evaluator) => `<div class="criterion-evaluator ${evaluator.grade == null ? "missing" : ""}"><div><strong>${h(evaluator.evaluatorName)}</strong><span class="role-badge ${h(evaluator.evaluatorRole)}">${h(categoryName(evaluator.evaluatorRole))}</span></div><div class="criterion-evaluator-actions"><div class="criterion-grade"><strong>${evaluator.grade == null ? "Missing" : `${fmt(evaluator.grade)} /${criterion.maximum}`}</strong>${evaluator.rawValue != null ? `<small>Result: ${h(evaluator.rawValue)}${criterion.unit ? ` ${h(criterion.unit)}` : ""}</small>` : ""}<small>${h(statusLabel(evaluator.status))}</small></div>${evaluator.submissionId ? `<button type="button" class="button secondary small edit-dimension-submission" data-id="${evaluator.submissionId}">Edit evaluation</button>` : ""}</div></div>`).join("") : `<p class="subtle">No published evaluator assignment for this activity.</p>`}</div>
        </article>`).join("")}</div>
    </section>`).join("");
...
function wireProfile(profile) {
  ...
  const payload = baseVersion => {
    ...
    const values = Object.fromEntries((state.system?.generalFactors || []).map((factor) => {
      const raw = data.get(`factor:${factor.storageKey}`);
      return [factor.storageKey, raw === "" || raw == null ? null : Number(raw)];
    }));
    return { values, comment: data.get("comment") || "", notes: data.get("notes") || "", base_version: baseVersion };
  };
...
async function showAdminEvaluationEditor(recruitId, activityCode) {
  try {
    ...
    const values = detail.rubric.kind === "sport" ? (existing?.raw || {}) : (existing?.responses || {});
...
async function renderPermissions() {
  ...
  const permissionPresets = accountPermissionPresets();
  const accountCreationDisabled = permissionPresets.length ? "" : `disabled title="Configure an enabled non-owner permission preset first"`;
  const permissionPresetWarning = permissionPresets.length ? "" : `<div class="warning-box"><strong>Account creation needs a permission preset.</strong><br>Add and enable at least one non-owner profile under Configure Assessment System → Pages &amp; access. Existing accounts and their permissions are unchanged.</div>`;
  ...
  if ($("#addAccount") && permissionPresets.length) $("#addAccount").onclick = addAccountDialog;
  if ($("#generateAccounts") && permissionPresets.length) $("#generateAccounts").onclick = async () => {
...
function addAccountDialog() {
  const presets = accountPermissionPresets();
  if (!presets.length) {
    openModal(`<div><h2>Set up a permission preset first</h2><p class="muted">Add and enable at least one non-owner profile under Configure Assessment System → Pages &amp; access. The preset supplies the new account's starting permissions.</p><div class="modal-actions"><button type="button" class="button primary" id="cancelModal">Close</button></div></div>`);
    $("#cancelModal").onclick = closeModal;
    return;
  }
  openModal(`<form id="accountForm"><h2>Add account</h2><div class="stack"><label>Username / nickname<input name="username" required maxlength="200"></label><label>Full name (optional)<input name="fullName" maxlength="200"></label><label>Phone number (optional)<input name="phoneNumber" inputmode="tel" maxlength="40"></label><label>Temporary password<input name="password" type="password" minlength="8" required></label><label>Assessor category<select name="role">${categoryOptions()}</select></label><label>Permission preset<select name="accessProfile" required>${accessProfileOptions()}</select><small class="muted">Sets this account's starting access. You can edit its permissions afterward.</small></label></div><div class="modal-actions"><button type="button" class="button ghost" id="cancelModal">Cancel</button><button class="button primary">Add account</button></div></form>`);
  $("#cancelModal").onclick = closeModal;
...
function openAccountInfoDialog(account) {
  openModal(`<form id="accountInfoForm"><p class="eyebrow">Account details</p><h2>Edit ${h(account.username)}</h2><div class="stack"><label>Username / nickname<input name="username" value="${h(account.username)}" required maxlength="200" ${account.isOwner ? "readonly" : ""}></label><label>Full name (optional)<input name="fullName" value="${h(account.fullName || "")}" maxlength="200"></label><label>Phone number (optional)<input name="phoneNumber" value="${h(account.phoneNumber || "")}" inputmode="tel" maxlength="40"></label><label>Assessor category<select name="role" ${account.isOwner ? "disabled" : ""}>${categoryOptions(account.evaluatorRole)}</select></label><label class="checkbox-inline"><input name="canEvaluate" type="checkbox" ${account.canEvaluate ? "checked" : ""} ${account.isOwner ? "disabled" : ""}> Can use the evaluator workspace</label></div><div class="modal-actions"><button type="button" class="button ghost" id="cancelModal">Cancel</button><button class="button primary">Save account info</button></div></form>`);
  $("#cancelModal").onclick = closeModal;
  $("#accountInfoForm").onsubmit = async (event) => {
    ...
    const body = {
      ...
      evaluator_role: account.isOwner ? account.evaluatorRole : form.get("role"),
      can_evaluate: account.isOwner ? true : form.get("canEvaluate") === "on",
      base_version: account.version,
    };
...
initializeSystemUI().then(configuration => {
  ...
  dimensionOrder = configuration.dimensions.map(item => item.key);
  dimensionNames = Object.fromEntries(configuration.dimensions.map(item => [item.key, item.name]));
  dimensionMaximums = Object.fromEntries(configuration.dimensions.map(item => [item.key, Number(item.displayMaximum || 5)]));
  const firstActivity = configuration.activities?.[0]?.key;
  if (firstActivity) state.assignmentActivity = state.monitoringActivity = firstActivity;
